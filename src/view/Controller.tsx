...
import type { FunctionalComponent, PropType, SlotsType } from 'vue'
import {
  defineComponent,
  withModifiers,
} from 'vue'
...
export const Action: FunctionalComponent<ActionButton> = (props) => {
  const { value, label, icon, onSelect } = props
  return (
    <NTooltip trigger="hover" key={value}>
      {{
        trigger: () => (
          <NButton
            type="primary"
...
export default defineComponent({
  ...
  slots: Object as SlotsType<ControllerSlot>,
  render() {
    return (
      <div
        class={[
          'relative',
          this.active && 'outline outline-1px outline-[var(--primary-color)]',
        ]}
        onClick={() => {
          console.log('hhh')
        }}
